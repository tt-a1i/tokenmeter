...
var cmdHelps = map[string]cmdHelp{
	...
	"blocks": {
		...
		usage:       "tm blocks [options]",
		description: "Show 5-hour session blocks with token totals, cost, and active-window burn rate / projection.",
		options: []optionHelp{
			{"--active", "only show the active block"},
			{"--token-limit N|max", "annotate block usage against a token limit"},
			{"--session-length DURATION", "duration of one session block (default 5h)"},
			{"--json", "emit JSON instead of table"},
			{"--breakdown", "include model breakdown rows"},
		},
		examples: []string{"tm blocks", "tm blocks --active", "tm blocks --token-limit 500000"},
		seeAlso:  []string{"daily", "statusline"},
	},
	"statusline": {
		name:        "statusline",
		short:       "Claude Code statusline provider (stdin → stdout)",
		usage:       "tm statusline [options]",
		...
		options: []optionHelp{
			...
			{"--burn-rate-display MODE", "burn-rate display: off | emoji | text | emoji-text"},
			{"--no-color", "disable ANSI color output"},
		},
	...
	"analyze": {
		...
		options: []optionHelp{
			...
			{"--all-projects", "include all projects for tool errors and file churn"},
