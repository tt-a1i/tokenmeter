...
func TestClaudeLogWatcherDedupesSidechainReplayAcrossProcessFileCalls(t *testing.T) {
	dir := t.TempDir()
	...
	original := `{"type":"assistant","sessionId":"s","uuid":"msg-stream","requestId":"main-req","isSidechain":false,"timestamp":"2026-01-14T12:07:10Z","message":{"model":"claude-sonnet-4-6","usage":{"input_tokens":100,"output_tokens":50}}}` + "\n"
	...
	if _, err := f.WriteString(sidechain); err != nil {
		_ = f.Close()
		t.Fatalf("append sidechain: %v", err)
	}
