...
func TestParseCodexEntry_TokenCountFoldsReasoningOutputTokens(t *testing.T) {
	entry := codexLogEntry{
		Timestamp: "2026-01-14T12:07:16.785Z",
		Type:      "event_msg",
		Payload: json.RawMessage(`{
	...
	if events[0].Data.OutputTokens != 37 {
		t.Fatalf("OutputTokens=%d want 37", events[0].Data.OutputTokens)
	}
}
...
func TestCodexWatcher_LoadsSavedExecJSONReasoningOutputTokens(t *testing.T) {
	dir := t.TempDir()
	sessionID := "savedexec-reasoning-1111-1111-111111111111"
	path := filepath.Join(dir, "run-"+sessionID+".jsonl")
	writeLinesToFile(t, path,
		`{"type":"turn.completed","timestamp":"2026-01-02T03:04:05.000Z","model":"gpt-5.2-codex","usage":{"input_tokens":120,"output_tokens":30,"reasoning_output_tokens":7,"total_tokens":157}}`,
	)
	...
	if tokenEvents[0].Data.OutputTokens != 37 {
		t.Fatalf("OutputTokens=%d want 37 (output + reasoning)", tokenEvents[0].Data.OutputTokens)
	}
}
...
func TestCodexWatcher_LoadsSavedExecJSONDefaultsMissingModel(t *testing.T) {
	dir := t.TempDir()
	sessionID := "savedexec-model-1111-1111-111111111111"
	path := filepath.Join(dir, "run-"+sessionID+".jsonl")
	writeLinesToFile(t, path,
		`{"type":"turn.completed","timestamp":"2026-01-02T03:04:05.000Z","usage":{"input_tokens":100,"output_tokens":30,"total_tokens":130}}`,
	)
	...
	if tokenEvents[0].Data.Model != "gpt-5" {
		t.Fatalf("Model=%q want gpt-5", tokenEvents[0].Data.Model)
	}
}
